...
if Config.Minigame == 'memorygame' then
	...
	function PoliceAlert()
		if Config.Illegal then
			...
			elseif Config.Dispatch == 'qb-core' then
				...
				print('[rpatty-dumpster] Config.Dispatch does not have a valid argument')
			end
		end
	end
	...
	TriggerServerEvent('rpatty-dumpster:server:SetEntity', ObjToNet(ent), true)
	if not Config.ResetOnReboot then TriggerServerEvent('rpatty-dumpster:server:ResetEntity', ObjToNet(ent)) end
	isBusy = false
